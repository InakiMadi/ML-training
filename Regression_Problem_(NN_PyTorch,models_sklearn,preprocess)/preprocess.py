...
def preprocess(dataframe, validation_df, combine):
    # print(len(dataframe.columns.values), end=separation)
    ## 81 features.
    # print(dataframe.head(7), end=separation)

    # print(dataframe.info(), end=separation)
    ## 81 features. 1460 entries.

    # object_features = dataframe.select_dtypes(include=['object']).columns.tolist()
    # print(len(object_features),end=separation)
    ## 43 features are 'object'. Can be categorical, can be strings.
    ...
    '''
            Get the numeric features to standardize.
    '''
    # print(dataframe[dataframe.columns[:7]].head(), end=separation)
    numeric_features = dataframe.select_dtypes(include=['int', 'float']).columns.drop('SalePrice')
    # print(numeric_features, end=separation)
    ## For training fit and transform, for test only transform.
    scaler = StandardScaler()
    dataframe[numeric_features] = scaler.fit_transform(dataframe[numeric_features])
    validation_df[numeric_features] = scaler.transform(validation_df[numeric_features])
    # print(dataframe[dataframe.columns[:7]].head(), end=separation)

    '''
            Feature selection.
    '''
    numeric_features_and_y = numeric_features.union(['SalePrice'])
    drop_features = ['1stFlrSF', 'GarageYrBlt', 'GrLivArea', 'GarageArea']
    # new_numeric_features = numeric_features_and_y.drop(drop_features)
    # print(new_numeric_features, end=separation)
    # corr_matrix = dataframe[new_numeric_features].corr()
    # print(corr_matrix,end=separation)
    # plt.figure(figsize=(10, 6))
    # sns.heatmap(corr_matrix, annot=True, cmap='coolwarm', vmin=-1, vmax=1)
    # plt.title("Correlation Matrix")
    # plt.show()

    features_and_y = features.drop(drop_features)
    features = features_and_y.drop(['SalePrice'])
    # print(features, end=separation)

    features = ['LotArea', 'YearBuilt', '1stFlrSF', '2ndFlrSF', 'FullBath', 'BedroomAbvGr', 'TotRmsAbvGrd', 'SalePrice']

    '''
        One-hot to get categorical features.
    '''
    # print(len(dataframe[features].select_dtypes(include=['O']).columns.tolist()), end=separation)
    # print(dataframe[features].head(),end=separation)
    new_dataframe = pd.get_dummies(dataframe[features])
    new_object_features = new_dataframe.select_dtypes(include=['object']).columns.tolist()
    # print(len(new_object_features),end=separation)
    ## 0 object features, therefore categorical features one-hot encoded properly.

    # print(dataframe.head(), end=separation)
    # print(new_dataframe.head(), end=separation)
    # print(len(dataframe.columns.values), end=separation)
    # print(len(new_dataframe.columns.values), end=separation)
    ## From 81 to 289 features due to One-hot encoding.
    # print(new_dataframe.info(), end=separation)
    ## Features: 251 bool, 3 float64, 35 int64.

    dataframe = new_dataframe
    # print(dataframe[features].head(),end=separation)
    return dataframe[features], validation_df[features[:-1]], [dataframe[features], validation_df[features[:-1]]]


def old_preprocess(dataframe, validation_df, combine):
    '''
